...
import { AlertCircle, ArrowLeft, Clock, FileText, Info, Eye } from "lucide-react"
...
export default function JoinAssessment() {
  ...
  const translations = {
    fr: {
      ...
      training_description: "Entraînez-vous sans limite de temps ni restriction",
      summative_description: "Évaluation notée avec surveillance et restrictions",
      no_time_limit: "Pas de limite de temps",
      unlimited_attempts: "Tentatives illimitées",
      good_to_know: "Bon à savoir",
      training_info: "Les résultats ne seront pas comptabilisés dans votre note finale",
      fixed_duration: "Durée limitée",
      single_attempt: "Une seule tentative",
      important: "Important",
      summative_warning: "Cette évaluation sera surveillée et comptabilisée dans votre note finale",
      ...
      understand_monitoring: "Je comprends que cette évaluation sera surveillée",
      ...
      warning_message: "Vous ne pourrez pas revenir en arrière après avoir soumis cette question",
      java_question: "Quelle est la différence entre une classe abstraite et une interface en Java?",
      option1: "Une classe abstraite peut avoir des méthodes implémentées, une interface ne peut pas",
      option2: "Une interface peut être instanciée, une classe abstraite ne peut pas",
      option3:
    ...
    en: {
      ...
      summative_description: "Graded assessment with monitoring and restrictions",
      ...
      important: "Important",
      summative_warning: "This assessment will be monitored and counted towards your final grade",
      ...
      warning_message: "You will not be able to go back after submitting this question",
      java_question: "What is the difference between an abstract class and an interface in Java?",
      option1: "An abstract class can have implemented methods, an interface cannot",
      option2: "An interface can be instantiated, an abstract class cannot",
      option3: "A class can inherit from multiple abstract classes, but can only implement one interface",
  ...
  const assessmentDetails = {
    ...
    duration: assessmentType === "practice" ? (language === "fr" ? "Illimité" : "Unlimited") : "1h30",
    questions: 15,
    attempts: assessmentType === "practice" ? (language === "fr" ? "Illimitées" : "Unlimited") : "1",
    restrictions:
      assessmentType === "summative"
        ? [
            language === "fr" ? "Copier-coller désactivé" : "Copy-paste disabled",
            language === "fr" ? "Changement d'onglet/fenêtre interdit" : "Tab/window switching prohibited",
            language === "fr" ? "Surveillance par webcam activée" : "Webcam monitoring enabled",
            language === "fr" ? "Accès aux ressources externes bloqué" : "External resources access blocked",
          ]
        : [],
  }
      ...
      <main className="flex-1">
        <div className="container py-6">
          <div className="flex items-center gap-2 mb-8">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight">{translate("join_assessment_title")}</h1>
            ...
            <div>
              <Tabs value={assessmentType} onValueChange={setAssessmentType} className="space-y-4">
                <TabsList className="grid grid-cols-2">
                  <TabsTrigger value="practice">{translate("training_assessment")}</TabsTrigger>
                  <TabsTrigger value="summative">{translate("summative_assessment")}</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="practice" className="space-y-4">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>{translate("training_description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-center gap-2">
                          <Clock className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{translate("no_time_limit")}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <FileText className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{translate("unlimited_attempts")}</span>
                        </div>
                      </div>
                      <Alert className="bg-blue-50 border-blue-200">
                        <Info className="h-4 w-4 text-blue-600" />
                        <AlertTitle className="text-blue-600">{translate("good_to_know")}</AlertTitle>
                        <AlertDescription className="text-blue-600">{translate("training_info")}</AlertDescription>
                      </Alert>
                    </CardContent>
                  </Card>
                </TabsContent>
                ...
                <TabsContent value="summative" className="space-y-4">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>{translate("summative_description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-center gap-2">
                          <Clock className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{translate("fixed_duration")}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <FileText className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{translate("single_attempt")}</span>
                        </div>
                      </div>
                      <Alert className="bg-amber-50 border-amber-200">
                        <AlertCircle className="h-4 w-4 text-amber-600" />
                        <AlertTitle className="text-amber-600">{translate("important")}</AlertTitle>
                        <AlertDescription className="text-amber-600">{translate("summative_warning")}</AlertDescription>
                      </Alert>
                ...
                <CardContent>
                  <form onSubmit={handleCodeSubmit} className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="assessment-code">{translate("assessment_code")}</Label>
                      <Input
                        id="assessment-code"
          ...
          <Dialog open={showPreview} onOpenChange={setShowPreview}>
            <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
                ...
                <div className="p-4 border-b sticky top-0 bg-white dark:bg-gray-950 z-10">
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-red-500">{translate("time_remaining")}: 45:23</span>
                    </div>
                ...
                <div className="p-6 space-y-6">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-xl font-semibold">{translate("java_question")}</h2>
                    </div>

                    <Alert className="bg-amber-50 border-amber-200">
                      <AlertCircle className="h-4 w-4 text-amber-600" />
                      <AlertTitle className="text-amber-600">{translate("warning_title")}</AlertTitle>
                      <AlertDescription className="text-amber-600">{translate("warning_message")}</AlertDescription>
                    </Alert>

                    <div className="pt-4">
                      <h3 className="text-sm font-medium mb-3">{translate("select_answer")}</h3>
              ...
              <div className="border-t p-4 flex justify-between">
                <Button variant="outline">{translate("previous_question")}</Button>
